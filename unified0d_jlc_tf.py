...
def wrap_to_2pi(angle):
    # function to map angles to a magnitude less than pi
    cond = lambda angle: tfm.greater(tfm.abs(angle), 2*pi)
    fn_true = lambda: angle - 4*pi
    fn_false = lambda: angle + 4*pi
    body = lambda angle: tf.cond(tfm.greater(angle, 2*pi), lambda: angle - 4*pi, lambda: angle + 4*pi)
    [angle_new] = tf.while_loop(cond, body, [angle])
    #tf.print(angle_new, [angle_new], "angle_new")
    return angle_new
...
def junction_loss_coeff_tf(U, A, theta):
    ...
    #tf.print(flow_rate, [flow_rate], "flow_rate")
    inlets = tf.greater_equal(flow_rate, tf.constant([0], dtype= "float32")) # identify inlets
    outlets = tf.less(flow_rate, tf.constant([0], dtype= "float32")) # identify outlets
    #tf.print(theta, [theta], "theta")
    # Angle Manipulations ------------------------------------------------
    ...
    #tf.print(pseudo_outlet_angle, [pseudo_outlet_angle], "pseudo_outlet_angle")
    #tf.print(theta, [theta], "theta")
